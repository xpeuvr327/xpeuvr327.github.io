...
function extractLifetimeMessagesData(conversationText) {
    ...
    lines.forEach(line => {
        ...
        if (match) {
            ...
            if (lifetimeMessagesData[sender]) {
                lifetimeMessagesData[sender].messageCount++;
            } else {
                lifetimeMessagesData[sender] = { sender, messageCount: 1 };
            }
        }
    });

    return Object.values(lifetimeMessagesData);
}
...
function updateLifetimeMessagesChart(lifetimeMessagesData) {
    ...
    // Prepare data for the chart
    const chartData = {
        labels: lifetimeMessagesData.map(data => data.sender),
        datasets: [{
            label: 'Lifetime Messages',
            data: lifetimeMessagesData.map(data => data.messageCount),
            backgroundColor: 'rgba(255, 99, 132, 0.2)', // Red background
            borderColor: 'rgba(255, 99, 132, 1)', // Red border
            borderWidth: 1
        }]
    ...
    // Create or update the chart
    const ctx = lifetimeMessagesChartDiv.getContext('2d');
    new Chart(ctx, {
        type: 'bar',
        data: chartData,
        options: {
