...
function processConversation(conversationText) {
    ...
    updateGraphicsUI(onlineStatusData, memberStatistics); // Pass memberStatistics as well
}
...
function updateMemberStatisticsUI(memberStatistics) {
    ...
    memberNamesDiv.innerHTML = `Member Names: ${memberStatistics.memberNames.map(name => `"${name}"`).join(', ')}`;
}
...
function updateGraphicsUI(onlineStatusData, memberStatistics) {
    ...
    const chartData = {
        labels: onlineStatusData.map(entry => entry.timestamp.toLocaleString()), // Use toLocaleString for full date and time
        datasets: memberStatistics.memberNames.map(member => {
