...
document.getElementById('timer').textContent = `Temps: 0 secondes`;
	...
	flagCount.textContent = "Drapeaux : 0" + " / " + mines;
}
...
function flagCount(mines) {
    // Récupérer l'élément <p> par son id
    const flagCount = document.getElementById("flag-count");
    // Calculer le nombre de drapeaux déjà placés sur la grille
    let flags = 0;
    ...
    // Calculer le nombre de drapeaux qu'il reste à placer
    let remaining = mines - flags;
    // Mettre à jour le texte de l'élément <p> avec le format "Drapeaux : x / y"
    flagCount.textContent = "Drapeaux : " + flags + " / " + mines;
}
...
function updateTimer() {
    if (startTime !== 0 && !gameOver) {
        ...
        const elapsed = (currentTime - startTime) / 1000; // Calculer le temps écoulé en secondes
        document.getElementById('timer').textContent = `Temps: ${elapsed.toFixed(2)} secondes`; // Afficher le temps avec deux décimales
    }
}
